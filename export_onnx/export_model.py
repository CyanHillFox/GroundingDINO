...
def export_model(model: torch.nn.Module, output_file: Union[str, pathlib.Path],
    ...
    with torch.no_grad(), ExportFlag(True),\
        ...
        torch.onnx.export(wrapped_model,
                          ...
                          input_names=["samples", "input_ids", "token_type_ids",
                                       "text_token_mask", "text_self_attention_masks", "position_ids"],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", "-c", type=pathlib.Path,
    ...
    # )
    parser.add_argument("--engine_path", "-o", type=pathlib.Path,
                        required=True, help="path to output tensorrt engine file")
    parser.add_argument("--optimize", action="store_true",
                        help="optimize exported onnx model when simplifying")
    ...
    engine_path: pathlib.Path = args.engine_path

    # output_dir: pathlib.Path = args.output_dir
    output_dir, model_name = os.path.split(engine_path)
    ...
    opt_model_path = os.path.join(output_dir, f"{model_name}.onnx")
